...
class ${className}FragmentUnitTest {
    ...
    fun onCreate_shouldCall_fetch${className}Data() {
        ...
        fragment.interactor = interactorSpy

        // When
        fragment.fetch${className}Data()
    ...
    fun onCreate_Calls_fetch${className}Data_withCorrectData() {
        ...
        // Then
        // Assert.assertTrue(interactorSpy.requestCopy.context)
    }

    private inner class ${className}InteractorSpy : ${className}BusinessLogic {

        var fetch${className}DataIsCalled = false
        lateinit var requestCopy: ${className}.${className}Data.Request
